...
/**
 * Standalone PHP unit-test bootstrap.
 *
 * The PHP unit tests only depend on a few Magento contracts, which they mock or
 * implement. This bootstrap autoloads the module's own classes and defines minimal
 * stand-ins for those contracts *only when the real framework is absent*, so the
 * tests run in plain CI (no Magento install). Inside a real Magento install the
 * genuine classes exist and win.
 */

namespace {
    spl_autoload_register(static function ($class) {
        $prefix = 'Softcode\\DawaAddress\\';
...
namespace Magento\Framework\Option {
    if (!interface_exists(ArrayInterface::class)) {
